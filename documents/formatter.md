...
VS Code から整形要求
  └─> 整形ツール（jar）の存在確認
        ...
        └─> resolveJavaHome()（同梱Javaの展開・設定値の取得）
              ...
              └─> java を起動し、ドキュメント全文を標準入力へ渡す
                    ├─ 失敗（構文エラー等） → 標準エラー出力の内容をエラー表示して終了
                    └─> 標準出力から整形結果を取得
                          ├─ 変化なし → 編集を行わない
...
| 項目 | 内容 |
|------|------|
| 入力の受け渡し | 標準入力（一時ファイルを作らない） |
| 改行コード | 整形結果の改行を、元のドキュメントの設定（`document.eol`）に合わせて変換する。CRLF のファイルを整形しても改行コードが変わらない |
| 戻り値 | 変化があれば「ファイル全体を置き換える TextEdit 1件」、無ければ空配列 |
| 失敗時 | エラーメッセージを表示し、空配列を返す（ドキュメントは変更されない） |
...
末尾の `-` は「標準入力から読み込み、標準出力へ出力する」指定。

| 項目 | 内容 |
|------|------|
| 実行方法 | `child_process.spawn`（非同期）。`windowsHide: true` でコンソール画面を出さない |
| 入力 | 標準入力へドキュメント全文を UTF-8 で書き込み、`end()` で閉じる |
| 標準出力 | 整形後のソースコード全文（UTF-8 として解釈する） |
| 標準エラー出力 | 構文エラー等。**読み捨てずに受け取る**（読まないとパイプが詰まって処理が止まるため）。失敗時はこの内容をエラーメッセージとして表示する |
| タイムアウト | 30秒。超過した場合はプロセスを終了して失敗として扱う |

`--offset` / `--length` は 0 起点の文字位置と文字数。公式の使用方法に従い、
両者は同じ回数だけ指定する必要があり、指定した場合は対象を1つ（ここでは `-`）に限る。
...
| 応答が30秒以内に得られない | プロセスを終了し、`The formatter did not respond.` を表示して中断 |
| 標準入力への書き込みに失敗 | エラーメッセージを表示して中断 |
